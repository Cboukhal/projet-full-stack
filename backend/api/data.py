# Comptes de démonstration créés par la migration 0001_initial (voir seed_demo_users).
# Un compte par rôle, avec un mot de passe en clair ici : il est haché par make_password
# au moment de l'insertion en base, jamais stocké tel quel.
USERS = [
    {
        "identifiant": "camille.dubois",
        "motDePasse": "eleve123",
        "role": "eleve",
        "nom": "Camille Dubois",
        "email": "camille.dubois@mail.fr",
        "telephone": "06 12 34 56 78",
    },
    {
        "identifiant": "julien.marchand",
        "motDePasse": "formateur123",
        "role": "formateur",
        "nom": "Julien Marchand",
        "email": "j.marchand@organisme.fr",
        "telephone": "06 98 76 54 32",
        "specialite": "Développement Java",
    },
    {
        "identifiant": "marie.petit",
        "motDePasse": "referente123",
        "role": "referente",
        "nom": "Marie Petit",
        "email": "m.petit@organisme.fr",
    },
    {
        "identifiant": "sophie.lenoir",
        "motDePasse": "admin123",
        "role": "administrateur",
        "nom": "Sophie Lenoir",
        "email": "s.lenoir@organisme.fr",
    },
]
